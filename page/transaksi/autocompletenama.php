...
$sql="SELECT * FROM tb_anggota WHERE nis LIKE '%".$searchTerm."%' ORDER BY nama ASC"; // query sql untuk menampilkan data mahasiswa dengan operator LIKE

$hasil=mysqli_query($kon,$sql); //Query dieksekusi

//Disajikan dengan menggunakan perulangan
while ($nama = mysqli_fetch_array($hasil)) {
    $data [] =$nama['nama'];
}
